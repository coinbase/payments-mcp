...
export class ConfigService {
  ...
  displayConfigInstructions(config: ClaudeDesktopConfig): void {
    ...
    this.logger.info(
      'To complete the setup, add the following configuration to your Claude Desktop settings:'
    );
    this.logger.newline();
    ...
    this.logger.newline();
    this.logger.info(
      'After restarting Claude Desktop, you can use the payments-mcp functionality in your conversations.'
    );
    this.logger.separator();
  }
  ...
  validateConfig(config: ClaudeDesktopConfig): boolean {
    try {
      ...
      if (
        config.isUsingBuiltInNodeForMcp !== undefined &&
        typeof config.isUsingBuiltInNodeForMcp !== 'boolean'
      ) {
        return false;
      }
  ...
  mergeWithExistingConfig(
    ...
    const merged = { ...existingConfig } as unknown as ClaudeDesktopConfig;
