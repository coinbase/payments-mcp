...
export class InstallService {
  ...
  async runNpmInstall(projectPath: string): Promise<void> {
    ...
    try {
      if (
        !(await this.fileUtils.exists(path.join(projectPath, 'package.json')))
      ) {
  ...
  async runElectronInstaller(projectPath: string): Promise<void> {
    ...
    try {
      ...
      if (!(await this.fileUtils.exists(electronDir))) {
        throw new Error(
          'Electron module not found. Make sure npm install completed successfully.',
        );
      }

      if (!(await this.fileUtils.exists(installerScript))) {
        this.logger.warn(
          'Electron install.js not found, skipping electron installer step',
        );
        return;
      }
  ...
  async verifyInstallation(projectPath: string): Promise<boolean> {
    try {
      ...
      if (!packageJson.name || !packageJson.version) {
        this.logger.debug(
          'Verification failed: Invalid package.json structure',
        );
        return false;
      }
  ...
  async getInstalledPackageInfo(
    ...
    try {
      ...
      this.logger.debug(
        `Failed to get package info: ${(error as Error).message}`,
      );
      return null;
    }
  }
  ...
  async checkNpmAvailability(): Promise<boolean> {
    try {
      ...
      this.logger.debug(
        `npm availability check failed: ${(error as Error).message}`,
      );
      return false;
    }
  }
  ...
  async checkNodeAvailability(): Promise<boolean> {
    try {
      ...
      this.logger.debug(
        `Node.js availability check failed: ${(error as Error).message}`,
      );
      return false;
    }
  }
  ...
  async cleanupFailedInstallation(projectPath: string): Promise<void> {
    try {
      ...
      this.logger.warn(
        `Failed to cleanup installation: ${(error as Error).message}`,
      );
    }
  }
}
